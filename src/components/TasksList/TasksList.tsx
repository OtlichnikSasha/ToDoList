  ...
  return (
    <section className={styles.wrapper}>
      <div className={styles.wrapper_list}>
        <div className={styles.wrapper_list__top_place}>
          <h1 className={styles.wrapper_list__top_place_heading}>To Do</h1>
          <div className={styles.wrapper_list__top_place_icon} onClick={toggleSwitch}>
            {settingsIcon}
          </div>
        </div>
        <AnimatePresence>
          {showSwitch ?
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className={styles.switch_place}
            >
              <p className={styles.switch_place__heading}>Show news?</p>
              <Switch onChange={changeNewsVisible} defaultChecked checked={isNewsVisible} />
            </motion.div>
            :
            <></>
          }
        ...
        <div className={styles.tasks}>
          {tasks.length ?
            ...
            :
            <h3 className={styles.first_task}>
              У вас ещё нет тасков! Самое время создать новый 😉
            </h3>
          }
         <CreateTask />
        </div>
